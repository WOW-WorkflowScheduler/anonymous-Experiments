...
kubectl cp chip_seq_data.csv download-pod:/nfs/chipseq/input
kubectl cp chip_seq_data_small.csv download-pod:/nfs/chipseq/input
# download chipseq git repository
kubectl exec download-pod -- /bin/sh -c "`cat git-commands.sh`"
# overwrite chipseq config
kubectl cp modules.config download-pod:/nfs/chipseq/chipseq/conf/modules.config

# copy list of accession numbers to download-pod
kubectl cp accession_nb.txt download-pod:/nfs/chipseq/input
