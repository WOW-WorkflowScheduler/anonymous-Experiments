...
kubectl cp input.csv download-pod:/nfs/sarek/input
kubectl cp inputSmall.csv download-pod:/nfs/sarek/input
# download sarek git repository
kubectl exec download-pod -- /bin/sh -c "`cat git-commands.sh`"

# copy list of samples to download-pod
kubectl cp samples.txt download-pod:/nfs/sarek/input
kubectl cp samplesSmall.txt download-pod:/nfs/sarek/input
